		...
		for (let attempt = 1; attempt <= 3; attempt++) {
			...
			while (safety++ < 20) {
				...
				if (loc && loc.startsWith('ondus://')) {
					...
					const tokenUrl =
						`https://idp2-apigw.cloud.grohe.com/v3/iot/oidc/token?code=${encodeURIComponent(code)}` +
						`&state=${encodeURIComponent(state)}`;

					this._logDebug(`GET token exchange ${safeHostPath(tokenUrl)} (query hidden)`);
					const tokenResp = await this.client.get(tokenUrl, { headers: { Accept: 'application/json' } });

					this._logDebug(
						`token status=${tokenResp.status} keys=${tokenResp.data ? Object.keys(tokenResp.data).join(',') : 'none'}`,
					);

					if (tokenResp.status !== 200) {
						throw new Error(`Token-Austausch fehlgeschlagen (${tokenResp.status})`);
					}
					if (!tokenResp.data?.access_token || !tokenResp.data?.refresh_token) {
						throw new Error('Token-Antwort unvollständig (access_token/refresh_token fehlt)');
					}
